"""Build/run the focused asset integration check; --prepare-builtins writes missing builtin files."""
import os
...
build = root / 'Tools/bin/AssetSystemCheck'
build.mkdir(parents=True, exist_ok=True)
project = build / 'AssetSystemCheck.vcxproj'
...
<ItemGroup><ClCompile Include="{root / 'Tools/AssetSystemCheck.cpp'}" /></ItemGroup>
...
subprocess.run([str(build / 'AssetSystemCheck.exe'), *sys.argv[1:]], cwd=root, env=env, check=True, timeout=60)
