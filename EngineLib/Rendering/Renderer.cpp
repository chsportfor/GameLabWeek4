...
{
	D3D11_BUFFER_DESC indexbufferdesc = {};
	indexbufferdesc.ByteWidth = sizeof(QuadTextureIndexedIndices);
	indexbufferdesc.Usage = D3D11_USAGE_IMMUTABLE;
	indexbufferdesc.BindFlags = D3D11_BIND_INDEX_BUFFER;
	indexbufferdesc.CPUAccessFlags = 0;

	D3D11_SUBRESOURCE_DATA indexbufferSRD = {};
	indexbufferSRD.pSysMem = QuadTextureIndexedIndices;
...
void URenderer::Release()
{
    ...
    if (LoadingScreenVertexBuffer) { LoadingScreenVertexBuffer->Release(); LoadingScreenVertexBuffer = nullptr; }
    if (LoadingScreenIndexBuffer) { LoadingScreenIndexBuffer->Release(); LoadingScreenIndexBuffer = nullptr; }
    if (LoadingScreenInputLayout) { LoadingScreenInputLayout->Release(); LoadingScreenInputLayout = nullptr; }
    if (LoadingScreenVertexShader) { LoadingScreenVertexShader->Release(); LoadingScreenVertexShader = nullptr; }
    if (LoadingScreenPixelShader) { LoadingScreenPixelShader->Release(); LoadingScreenPixelShader = nullptr; }
    if (LoadingScreenSampler) { LoadingScreenSampler->Release(); LoadingScreenSampler = nullptr; }
...
void URenderer::RenderSimplePrimitive(ID3D11Buffer* Vertices, ID3D11Buffer* Indices, UINT IndexCount)
{
    UINT Offset = 0;
    DeviceContext->IASetVertexBuffers(0, 1, &Vertices, &StrideSimple, &Offset);
...
void URenderer::RenderParticle(ID3D11ShaderResourceView* texture)
{
	...
	// Bind the vertex buffer
	DeviceContext->IASetVertexBuffers(0, 1, &ParticleVertexBuffer, &StrideSimple, &offset);
...
void URenderer::RenderHighlight(ID3D11Buffer* pBuffer, ID3D11Buffer* Indices, uint32 Num, FMatrix mViewProjectionMatrix, FMatrix OutlineMatrix, const FMatrix originalMatrix)
{
	...
	RenderSimplePrimitive(pBuffer, Indices, Num);
...
{
    LoadingScreenVertexBuffer = CreateVertexBuffer(Fullscreen_vertices, static_cast<UINT>(std::size(Fullscreen_vertices))).Detach();
    LoadingScreenIndexBuffer = CreateIndexBuffer(Fullscreen_indices, static_cast<UINT>(std::size(Fullscreen_indices))).Detach();
...
void URenderer::RenderFullscreenTexture(ID3D11ShaderResourceView* texture)
{
	...
	DeviceContext->IASetIndexBuffer(LoadingScreenIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
	DeviceContext->DrawIndexed(static_cast<UINT>(std::size(Fullscreen_indices)), 0, 0);
}
...
bool URenderer::ensureFontIndexBuffer(UINT fontCpunt)
{
	...
	ID3D11Buffer* newBuffer = CreateIndexBuffer(indices.data(), static_cast<UINT>(indices.size())).Detach();
